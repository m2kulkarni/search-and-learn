...
"""Merge revisions of multiple datasets into a single config.
Usage:
# Merge all revisions of multiple datasets for a given seed
python scripts/merge_chunks.py \
...
def main():
    ...
    # Initialize lists to store all revisions and pool arguments
    all_revisions = []
    pool_args = []
    ...
    for dataset_name in args.dataset_name:
        revisions = get_dataset_revisions(dataset_name)
        if args.filter_strings:
            revisions = [
                revision
                for revision in revisions
        ...
        pool_args.extend([
            (dataset_name, revision, args.dataset_split) for revision in revisions
        ])

    # Use the first dataset's config as the base for merged config
    merged_config = all_revisions[0].split("--chunk")[0] if all_revisions else "merged"
    print(f"Merging {len(all_revisions)} total revisions to create config `{merged_config}`")
    ...
    if "MATH-500" in merged_config and len(merged_dataset) != 500:
        raise ValueError(f"Expected 500 samples, got {len(merged_dataset)}")
    if "gsm8k" in merged_config and len(merged_dataset) != 1319:
        raise ValueError(f"Expected 1319 samples, got {len(merged_dataset)}")

    # Push merged dataset to the hub
    # Using the first dataset name as the destination
    url = merged_dataset.push_to_hub(
        args.dataset_name[0],  # Use the first dataset name as the destination
        config_name=merged_config,
        split=args.dataset_split,
        private=args.hub_dataset_private,
    )
